...
class SyntheticBattlefieldGenerator:
    ...
    def generate_next_frame(self, enable_noise=True, enable_threats=True):
        ...
        # 2. Inject Camouflaged Threats (Anomalies)
        if enable_threats:
            # Update target 1 (Armored Asset moving across terrain)
            ...
            if threat1_roi.shape[0] == 40 and threat1_roi.shape[1] == 50:
                # Distort baseline geometry with metallic thermal signature
                cv2.rectangle(frame_uint8, (t1_x, t1_y), (t1_x+50, t1_y+40), (140, 170, 110), -1)
                cv2.circle(frame_uint8, (t1_x+25, t1_y+20), 12, (200, 220, 180), -1)

            # Update target 2 (Concealed personnel / Disturbed Earth)
            ...
            cv2.ellipse(frame_uint8, (t2_x+20, t2_y+20), (25, 15), 30, 0, 360, (90, 130, 160), -1)
